...
const actions = {
	async getConversationList({
		...
		try {
			const {
				data
			} = await IMSDK.compatibleAPI(IMSDK.IMMethods.GetConversationListSplit, uuidv4(),isFirstPage ? 0 : state.conversationList.length,10)
			commit('SET_CONVERSATION_LIST', [...(isFirstPage ? [] : state.conversationList),...data])
			return [...data]
		} catch (e) {
			console.log(e);
	...
	updateCurrentMemberInGroup({
		...
		// console.log(state.currentMemberInGroup);
