<template>
	<u-swipe-action-item :index="source.conversationID" @click="clickConversationMenu($event,source)"
		:name="source.conversationID" :options="getSwipeActions || []" @touchmove.stop >
			...
			<view class="left_info">
				...
				<view class="details">
					...
					<view class="lastest_msg_wrap">
						<text v-if="messagePrefix" class="lastest_msg_prefix"
							:class="{'lastest_msg_prefix_active':needActivePerfix}">{{messagePrefix}}</text>
	...
	export default {
		...
		computed: {
			messagePrefix() {
				...
				if (this.source?.recvMsgOpt !== OptType.Nomal && this.source.unreadCount > 0) {
