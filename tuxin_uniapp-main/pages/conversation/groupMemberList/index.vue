<template>
	<view @click="pageClick" class="group_members_container">
		<group-member-list-header ref="navHeaderRef" :checkVisible.sync="showCheck" :isTransfer="isTransfer"
			:isNomal="!isOwner&&!isAdmin" :isCall="type === 'CallInvite'" :groupID="groupID"
			@removeMember="showMemberCheck" />

		<view class="search_bar_wrap">
			<u-search :disabled="groupMemberList.length === 0" @clear="searchOrCancel" @custom="searchOrCancel"
				@search="searchMember" class="search_bar" shape="square" placeholder="搜索"
				:actionText="isSearching ? '取消' : '搜索'" v-model="keyword" />
		</view>
		...
		<choose-index-footer v-if="showCheck" :comfirmLoading="comfirmLoading" @removeItem="updateCheck"
			@confirm="confirm" :choosedData="getChoosedData" :isRemove="isRemove" />
...
<script>
	...
	export default {
			...
			canCheck() {
				return ({
					...
					if (this.type === GroupMemberListTypes.Kickout) {
						return (this.isOwner || (this.isAdmin && member.roleLevel === GroupMemberRole.Owner)) &&
		...
		onLoad(options) {
			...
			if (this.isRightKick || type === GroupMemberListTypes.ChooseAt || type === GroupMemberListTypes.CallInvite) {
				this.showCheck = true;
			}
			if (callMediaType) {
		...
		methods: {
			async pageClick(e) {
				...
				if (e.target.y < moreActionArea.top || e.target.y > moreActionArea.bottom ||
			...
			confirm() {
				if (this.type === GroupMemberListTypes.ChooseAt || this.type === GroupMemberListTypes.CallInvite) {
					...
					if (this.type === GroupMemberListTypes.ChooseAt) {
						...
						prevPage.$vm.sendRtcInvite(this.callMediaType, this.getChoosedData.map(item => item.userID))
					}
					uni.navigateBack({
						delta: 1
					})
			...
			modalConfirm() {
				let func = () => {}
				if (this.type === GroupMemberListTypes.Kickout) {
					func = IMSDK.compatibleAPI(IMSDK.IMMethods.KickGroupMember, IMSDK.uuidv4(), this.getChoosedData[0]
						.groupID, '', this
						.getChoosedData.map(member => member.userID))
				} else {
					func = IMSDK.compatibleAPI(IMSDK.IMMethods.TransferGroupOwner, IMSDK.uuidv4(), this
						.choosedTransferMember.groupID, this.choosedTransferMember.userID)
				}
				func.then(() => this.showToast('操作成功', () => uni
					.navigateBack())).catch(() => this.showToast('操作失败')).finally(() => this.showConfirmModal = false)
			},
			...
			userClick(member) {
				if (this.type === GroupMemberListTypes.Transfer) {
					...
					uni.$u.route('/pages/common/userCard/index', {
						...
						disableAdd: this.$store.getters.storeCurrentGroup.applyMemberFriend === GroupAllowTypes
							.NotAllowed
					})
				}
			},
			...
			loadMemberList(groupID) {
				...
				IMSDK.compatibleAPI(IMSDK.IMMethods.GetGroupMemberList, IMSDK.uuidv4(), groupID ?? this
						.groupID, 0, this.groupMemberList
						.length, 20)
					.then(({
						data
					}) => {
			...
			async loadSearchList() {
				...
				try {
					const {
						data
					} = await IMSDK.compatibleAPI(IMSDK.IMMethods.SearchGroupMembers, IMSDK.uuidv4(), options)
					console.log(data);
			...
			groupMemberInfoChangeHandler({
				...
				if (updatedInfo.groupID === this.groupMemberList[0]?.groupID) {
					const idx = this.groupMemberList.findIndex(member => member.userID === updatedInfo.userID)
					if (idx > -1) {
			...
			groupMemberAddedHandler({
				...
				const updatedInfo = JSON.parse(groupMemberInfo);
				if (updatedInfo.groupID === this.groupMemberList[0]?.groupID && this.groupMemberList.length < 20) {
					const idx = this.groupMemberList.findIndex(member => member.userID === updatedInfo.userID)
					if (idx === -1) {
						this.groupMemberList = [...this.groupMemberList, updatedInfo]
					}
				}
			},
			groupMemberDeletedHandler({
				groupMemberInfo
			}) {
				const updatedInfo = JSON.parse(groupMemberInfo);
				if (updatedInfo.groupID === this.groupMemberList[0]?.groupID) {
