<template>
	<view class="page_container">
		...
		<view class="pane_row" :style="{'transform': `translateX(${isReaded? '0': '-100%' })`}">
			<view class="pane_content">
				<u-list @scrolltolower="readedScrolltolower" :height="`${listHeight}px`" lowerThreshold="100" v-if="readedList.length>0"
					class="member_list">
			...
			<view class="pane_content">
				<u-list @scrolltolower="unreadScrolltolower" :height="`${listHeight}px`" lowerThreshold="100" v-if="unReadList.length>0"
					class="member_list">
	...
	export default {
		...
		data() {
			return {
				...
				listHeight: uni.getWindowInfo().windowHeight - uni.getWindowInfo().statusBarHeight - 110
			};
		},
		onLoad(options) {
			...
			this.tabList[1].name = `未读 ${this.messageState.memberCount - readCount - 1}`
		...
		methods: {
			...
			getReadedMemberList() {
				...
				const offsetList = this
					.messageState.hasReadUserIDList.slice(this.readedList.length, this.readedList.length + 20)
				IMSDK.compatibleAPI(IMSDK.IMMethods.GetGroupMembersInfo, IMSDK.uuidv4(), this.messageState.groupID,
						offsetList)
					.then(({
						data
					}) => {
			...
			getUnreadMemberList() {
				...
				IMSDK.compatibleAPI(IMSDK.IMMethods.GetGroupMemberListByJoinTimeFilter, IMSDK.uuidv4(), this.messageState
						.groupID, this.unReadList.length, 20, 0, (this.messageState.sendTime / 1000).toFixed(), [...this.messageState
						.hasReadUserIDList,this.$store.getters.storeCurrentUserID])
					.then(({
						data
					}) => {
