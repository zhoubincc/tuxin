<template>
	<scroll-view :scroll-with-animation="withAnimation" @click="click" id="scroll_view" :style="{height:'1px',backgroundImage:`url(${bgUrl})`,backgroundSize:`100% ${bgHeight}`}"
		...
		<view id="scroll_wrap">
			...
			<view v-for="(item,index) in storeHistoryMessageList" :key="item.clientMsgID">
				...
				<message-item-render :mutipleCheckVisible="mutipleCheckVisible" :menuOutsideFlag="menuOutsideFlag"
		...
		<transition name="fade">
			<view @click="scrollToBottom" v-show="getNewMesageCount && !needScoll" class="new_message_flag fade">
...
<script>
	...
	export default {
		...
		computed: {
			...
			getNewMesageCount() {
				return this.storeHistoryMessageList.filter(message => !message.isRead && message.isAppend).length
			},
			sendFailedDesc() {
				return (message) => {
					...
					if (message.status === MessageStatus.Failed && message.errCode === SendMessageFailedType
						.NotFriend) {
						return '李立白开启了好友验证，你还不是他（她）好友。请先发送好友验证，对方验证通过后，才能聊天。'
					}
					return ''
				}
			},
