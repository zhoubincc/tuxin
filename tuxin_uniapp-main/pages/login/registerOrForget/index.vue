	...
	<view class="register_container content_with_back">
		...
		minWidth: '200rpx'
			...
			<u-form-item prop="phoneNumber" label="手机号码" borderBottom>
				<u-input v-model="userInfo.phoneNumber" border="none" placeholder="请输入手机号码" clearable>
			...
			<u-form-item v-if="isRegister" prop="invitationCode" label="邀请码" borderBottom>
				<u-input v-model="userInfo.invitationCode" border="none" :placeholder="`请输入邀请码${needInvitationCodeRegister ? '(必填)' : '(选填)'}`" clearable>
				</u-input>
			</u-form-item>
		</u-form>
		<view class="action_btn">
			<u-button @click="sendSms" type="primary" :disabled="isRegister && !checked[0]">
				{{isRegister ? '立即注册' : '获取验证码'}}
			</u-button>
		</view>
		<view class="agreement" v-show="isRegister">
			<u-checkbox-group v-model="checked">
				<u-checkbox iconSize="12" labelSize="12" shape="circle" label="我已阅读并同意：" :name="true"></u-checkbox>
			</u-checkbox-group>
			...
			sendSms() {
				this.$refs.registerForm.validate().then((valid) => {
					...
					businessSendSms(options).then(() => {
						uni.$u.toast('验证码已发送！')
						setTimeout(() => uni.$u.route("/pages/login/verifyCode/index", {
