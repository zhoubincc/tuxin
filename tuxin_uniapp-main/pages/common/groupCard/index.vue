<template>
	<view class="group_card_container">
		...
		<view @click="toMemberList" class="member_row info_row">
		...
		methods: {
			toMemberList() {
				if (this.isJoinedGroup) {
					...
					this.$store.dispatch('conversation/getCurrentMemberInGroup', this.sourceID)
					uni.navigateTo({
			...
			joinGroup() {
				uni.$u.route('/pages/common/sendAddRequest/index',{
					...
					isScan: this.isScan,
					notNeedVerification: this.sourceGroupInfo.needVerification === GroupVerificationTypes.AllNot,
					sessionType: this.sourceGroupInfo.groupType === GroupType.NomalGroup ? SessionType.Group :
