	...
	export default {
		...
		methods: {
			...
			updateUnhandleNum() {
				...
				const numStoreKey = this.isGroupApplication ? 'storeUnHandleGroupApplicationNum' :
				...
				if (this[numStoreKey] > 0) {
					...
					const storageKey =
						`${currentUserID}_${this.isGroupApplication ? 'accessedGroupApplications' : 'accessedFriendApplications'}`
					const storeKey = this.isGroupApplication ? 'storeRecvGroupApplications' : 'storeRecvFriendApplications'

					const applicationKey = this.isGroupApplication ? 'userID' : 'fromUserID'
					const accessedData = uni.getStorageSync(storageKey) || []
					this[storeKey].forEach(application => {
						const item = `${application
									[applicationKey]}_${application
									.createTime}`
					...
					uni.setStorage({
						...
						data: accessedData,
						success: () => {
							const mutationMethod = this.isGroupApplication ? 'SET_RECV_GROUP_APPLICATIONS' :
								'SET_RECV_FRIEND_APPLICATIONS'
							this.$store.commit(`contact/${mutationMethod}`, [...this[storeKey]])
						}
					})
				}
			}
