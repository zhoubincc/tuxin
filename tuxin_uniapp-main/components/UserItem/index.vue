<template>
	<view @click="clickItem" class="user_item">
		...
		<my-avatar :src="item.faceURL" :desc="item.remark || item.nickname || item.showName" :isGroup="item.groupName !== undefined || GroupSessionTypes.includes(item.conversationType)"
			size="42" />
...
<script>
	...
	import { GroupSessionTypes } from '@/constant';
	export default {
		name: "UserItem",
		components: {
			MyAvatar
