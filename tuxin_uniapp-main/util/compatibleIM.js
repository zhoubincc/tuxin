...
const callingModule = uni.requireNativePlugin('OUICalling');
const filePicker = uni.requireNativePlugin('OUIFilePicker');
import {
	v4 as uuidv4
} from 'uuid';
import {
	...
	return new Promise((resolve, reject) => {
		...
		IMApi[key](...args, (res) => {
			if (res.errCode === 0) {
				...
				if (key === IMMethods.Login) {
					// Igexin.bindAlias(args[1])
				}
				resolve(res)
			} else {
				reject(res)
...
const IMMethods = {
	...
	SetOneConversationBurnDuration: "setOneConversationBurnDuration",
	DeleteConversation: "deleteConversation",
